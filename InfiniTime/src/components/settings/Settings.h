...
// Pulser getters and setters
      uint32_t GetPulse1() const {
        return settings.pulse1;
      };

      void SetPulse1(uint32_t pulse) {
        if (pulse != settings.pulse1) {
          settingsChanged = true;
        }
        settings.pulse1= pulse;
      };


      uint32_t GetPulse2() const {
        return settings.pulse2;
      };
      void SetPulse2(uint32_t pulse) {
        if (pulse != settings.pulse2) {
          settingsChanged = true;
        }
        settings.pulse2= pulse;
      };

      uint32_t GetPulse3() const {
        return settings.pulse3;
      };
      void SetPulse3(uint32_t pulse) {
        if (pulse != settings.pulse3) {
          settingsChanged = true;
        }
        settings.pulse3= pulse;
      };

      uint32_t GetPulse4() const {
        return settings.pulse4;
      };
      void SetPulse4(uint32_t pulse) {
        if (pulse != settings.pulse4) {
          settingsChanged = true;
        }
        settings.pulse4= pulse;
      };
//
	...
	uint32_t pulse4 = 140;
        ...
        Pinetime::Applications::WatchFace watchFace = Pinetime::Applications::WatchFace::Conductor;
